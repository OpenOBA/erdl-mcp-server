...
echo ""
echo "  ╔══════════════════════════════════════════╗"
echo "  ║  ERDL MCP Server — Quick Installer v1.0  ║"
...
if [ -d "$INSTALL_DIR" ]; then
  ...
  git pull --ff-only origin master 2>/dev/null || {
    echo "  Repository in conflict. Backing up to ${INSTALL_DIR}.bak..."
...
VERSION=$(node bin/erdl-mcp.js --version 2>&1 | tail -1)
echo "  ✓ $VERSION"
...
if [ -w "/usr/local/bin" ] || command -v sudo &> /dev/null; then
  ...
  echo "  ✓ erdl-mcp command available globally"
fi
...
if command -v openclaw &> /dev/null; then
  ...
  openclaw mcp set erdl "{\"command\":\"node\",\"args\":[\"$INSTALL_DIR/dist/index.js\"]}" 2>/dev/null && \
    echo "  ✓ OpenClaw MCP configured" || \
    echo "  ! Could not configure OpenClaw (you can do this manually later)"
fi

# ---- Done ----
echo ""
echo "  ╔══════════════════════════════════════════╗"
echo "  ║  ✓ ERDL MCP Server installed successfully ║"
echo "  ║                                          ║"
echo "  ║  Run:        erdl-mcp                    ║"
